...
def t_plot(mom, a_str, z_plot):
    ...
    ax.errorbar(np.arange(1, Nt)[:16], [val.mean for val in m_eff][:16], yerr=[val.sdev for val in m_eff][:16], label='Local', **errorb)
    ax.set_ylim([0.3, 0.5])
    ...
    fig = plt.figure(figsize=fig_size)
    ax = plt.axes(plt_axes_small)
    ax.errorbar(np.arange(1, Nt)[:16], [val.mean for val in m_eff_z[str(z_plot)]][:16], yerr=[val.sdev for val in m_eff_z[str(z_plot)]][:16], label='Non local, z='+str(z_plot)+', real', **errorb)
    ax.set_ylim([0.3, 0.5])
    ax.set_xlabel(t_label, **fs_p)
...
def pt2_fit(mom, a_str, t_ls, z_fit):
    ...
    fig = plt.figure(figsize=fig_size)
    ax = plt.axes(plt_axes_small)
    ax.errorbar(np.arange(1, Nt)[:14], [ (an_pi_bs_avg[z_fit, idt, 0] / an_pi_bs_avg[0, idt, 0]).mean for idt in range(0, Nt-1) ][:14], [ (an_pi_bs_avg[z_fit, idt, 0] / an_pi_bs_avg[0, idt, 0]).sdev for idt in range(0, Nt-1) ][:14], label='Data, z='+str(z_fit), **errorb)
...
if __name__ == "__main__":
    # bad one, choose one state fit
    mom=6
    a_str='a12'
    z=1
    t_ls=np.arange(4, 10)
    t_ls_ = np.arange(6, 9) # for one state fit
    ...
    # good one, can do two states fit
    # mom=6
    # a_str='a06'
    # z=1
    # t_ls=np.arange(6, 13)
    # t_ls_ = np.arange(11, 14)
